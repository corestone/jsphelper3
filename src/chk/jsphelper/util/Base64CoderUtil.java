...
public class Base64CoderUtil
{
	...
	static
	{
		...
		Base64CoderUtil.map1[i++] = '+';
		Base64CoderUtil.map1[i++] = '/';

		for (int j = 0; j < Base64CoderUtil.map2.length; j++)
		{
			Base64CoderUtil.map2[j] = -1;
		}
		for (int j = 0; j < 64; j++)
		{
			Base64CoderUtil.map2[Base64CoderUtil.map1[j]] = (byte) j;
		}
	}
	...
	{
		return Base64CoderUtil.decode(in, 0, in.length);
	}
	...
	public static byte[] decode (final char[] in, final int iOff, int iLen)
	{
		...
		while (ip < iEnd)
		{
			...
			final int b0 = Base64CoderUtil.map2[i0];
			final int b1 = Base64CoderUtil.map2[i1];
			final int b2 = Base64CoderUtil.map2[i2];
			final int b3 = Base64CoderUtil.map2[i3];
	...
	{
		return Base64CoderUtil.decode(s.toCharArray());
	}
	...
	public static byte[] decodeLines (final String s)
	{
		...
		return Base64CoderUtil.decode(buf, 0, p);
	}
	...
	{
		return new String(Base64CoderUtil.decode(s));
	}
	...
	{
		return Base64CoderUtil.encode(in, 0, in.length);
	}
	...
	{
		return Base64CoderUtil.encode(in, 0, iLen);
	}
	...
	public static char[] encode (final byte[] in, final int iOff, final int iLen)
	{
		...
		while (ip < iEnd)
		{
			...
			out[op++] = Base64CoderUtil.map1[o0];
			out[op++] = Base64CoderUtil.map1[o1];
			out[op] = op < oDataLen ? Base64CoderUtil.map1[o2] : '=';
			op++;
			out[op] = op < oDataLen ? Base64CoderUtil.map1[o3] : '=';
			op++;
		}
		return out;
	}
	...
	{
		return Base64CoderUtil.encodeLines(in, 0, in.length, 76, Base64CoderUtil.systemLineSeparator);
	}
	...
	public static String encodeLines (final byte[] in, final int iOff, final int iLen, final int lineLen, final String lineSeparator)
	{
		...
		while (ip < iLen)
		{
			...
			buf.append(Base64CoderUtil.encode(in, iOff + ip, l));
	...
	{
		return new String(Base64CoderUtil.encode(s.getBytes()));
	}
